...
<div class="song-left" onclick="selectSong(${index})">
<img src="${song.cover}">
<div class="song-info">
<div>${song.title}</div>
...
<button onclick="shareSong(${index})">🔗 Share</button>
<button id="download-${index}" onclick="downloadSong(${index})">⬇ Download</button>
</div>
</div>
`;
...
navigator.share({
title:song.title,
text:"Listen on VibeVerse 🎧",
url:song.file
});

}else{

navigator.clipboard.writeText(song.file);
